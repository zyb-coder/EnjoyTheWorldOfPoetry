...
class GameWindow(object):
    ...
    def __init__(self, length, wide, *args, **kw):
        self.window_length = length
        self.window_wide = wide
        # 绘制游戏窗口，设置窗口尺寸  NOFRAME-让窗口没有边框
        self.game_window = pygame.display.set_mode((self.window_length, self.window_wide), NOFRAME)
        # 设置游戏窗口标题
        pygame.display.set_caption("娱乐休闲")
        # 定义游戏窗口背景颜色参数
...
class Collision(object):
    ...
    def ball_rect(self):
        ...
        # 定义球拍到圆心最近点与圆心的距离
        self.distance = sqrt(
            pow(self.closestpoint_x - self.ball_x, 2) + pow(self.closestpoint_y - self.ball_y, 2))
    ...
    def ball_brick(self):
        ...
        # 定义砖块到圆心最近点与圆心的距离
        self.distanceb = sqrt(
            pow(self.closestpoint_bx - self.ball_x, 2) + pow(self.closestpoint_by - self.ball_y, 2))
...
class Main1(GameWindow, Rect, Ball, Brick, Collision, Score, Win, GameOver):
    ...
    def __init__(self, length, wide, *args, **kw):
        super(Main1, self).__init__(length, wide,*args, **kw)
        super(GameWindow, self).__init__(length, wide,*args, **kw)
